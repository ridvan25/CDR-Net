...
parser.add_argument('--model', type=str,  default="CDRNet_flod0", help='model_name')                # todo change test model name
...
def test_calculate_metric(epoch_num):
    ...
    v_save_mode_path = os.path.join(snapshot_path, 'vnet_best.pth')
    vnet.load_state_dict(torch.load(v_save_mode_path))
    print("init weight from {}".format(v_save_mode_path))
    vnet.eval()

    r_save_mode_path = os.path.join(snapshot_path, 'resnet_best.pth')
